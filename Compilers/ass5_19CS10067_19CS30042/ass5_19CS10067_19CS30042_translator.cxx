...
sym* symtable::lookup(string name){
    sym* symbol;
    list<sym>::iterator it = table.begin();                                                        // "it" is list iterator for list of symbols and start a linear search from the first element in the table
    while(it != table.end()){
        if(it->name != name) it++;                                                                 // Continue the search
        else return &(*it);                                                                        // If name of the symbol is found in the table then return the address of the element
    }
...
void symtable::update(){                                                                           // Update the symbol table and the offsets in it
    ...
    int off;
    list<sym>::iterator it = table.begin();                                                        // List iterator for elements in the symbol table
    while(it != table.end()){
        if(it == table.begin()){
    ...
    list<symtable*>::iterator it1 = tb.begin();                                                    // List iterator for the nested tables
    while(it1 !=tb.end()){                                                                         // Recursively update the nested tables 
        (*it1)->update();
        it1++;
    }
}
